...
loader.load('/scene.gltf',
    ...
    function createDropZone(x, z, color) {
        let dropZoneGeometry = new THREE.PlaneGeometry(5, 5);
    ...
    let dropZoneLeft = createDropZone(-7, -7, 0xff0000);
    let dropZoneMiddle = createDropZone(0, -7, 0x00ff00);
    let dropZoneRight = createDropZone(7, -7, 0x0000ff);

    let objects = [];
    const textureLoader = new THREE.TextureLoader();

// Function to create a text label using a canvas texture
function createTextLabel(text) {
    ...
    // Create sprite (acts as floating text)
    const sprite = new THREE.Sprite(material);
    sprite.scale.set(2, 1, 1); // Adjust text size
...
async function drawGreenSquares(userId, workerData) {
    ...
    if (workerData) {
        ...
        for (const worker of workerData) {
            ...
            cube.userData.id = `cube_${worker.workerId}`;
    ...
    function getDropZoneCategory(cube) {
        let zones = [
            { name: "left", zone: dropZoneLeft },
            { name: "middle", zone: dropZoneMiddle },
            { name: "right", zone: dropZoneRight }
        ];
    ...
    function setupDragControls() {
        ...
        dragControls.addEventListener("dragend", (event) => {
            ...
            if (dropZoneCategory) {
                ...
                assignCubeToCategory(cube.userData.id, dropZoneCategory);
                scene.remove(cube);
            }
        });
    }
